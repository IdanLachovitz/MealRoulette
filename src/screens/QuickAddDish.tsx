...
import type { ComponentType, Dish } from '../types'
import { COMPONENT_LABEL } from '../types'

/**
 * Fire-and-forget: a new dish gets its picture generated in the background
 * so adding a dish never waits on the network. Never throws — a sync-off or
 * failed generation just leaves the dish without a photo, same as before
 * this existed, and it can still be made from Settings or the dish's own
 * "regenerate" button. Re-reads the row before writing back, in case the
 * dish was edited (or its photo set another way) while generation was
 * still in flight — this must patch onto the latest version, not clobber
 * it with whatever the dish looked like at the moment it was added.
 */
function autoGenerateDishImage(dish: Dish): void {
  void generateDishImageWithAi(dish.name, dish.ingredients.map((i) => i.name)).then(async (imageUrl) => {
    if (!imageUrl) return
    const current = await db.dishes.get(dish.id)
    if (current && !current.image_url) await save('dishes', { ...current, image_url: imageUrl })
  })
}
  ...
  const addOne = async () => {
    ...
    try {
      if (kind === 'dish') {
        ...
        const created = await save('dishes', blankDish(householdId, trimmed, prep))
        autoGenerateDishImage(created)
      } else {
  ...
  const addBulk = async () => {
    ...
    try {
      ...
      if (kind === 'dish') {
        ...
        if (rows.length) {
          await saveMany('dishes', rows)
          rows.forEach(autoGenerateDishImage)
        }
